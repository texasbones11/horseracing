...
print(list(df))
df1 = df[['age', 'date', 'distance', 'notes',
          'race', 'sex', 'speed', 'time', 'track', 'type', 'winnings', 'speed_rating', 'starts', 'win', 'thirds', 'seconds']]

# remove space in notes:
df1['notes'] = df1['notes'].replace(' ', '', regex=True)

df2 = df1[['age', 'distance', 'notes', 'race', 'sex', 'speed',
           'time', 'track', 'type', 'winnings', 'speed_rating', 'starts', 'win', 'thirds', 'seconds']]

df2['race'].replace('', 'Raced', inplace=True)
df2['speed_rating'].replace('N/A', '', inplace=True)
...
df2 = df2[['sex', 'type', 'time', 'distance', 'speed',
           'race', 'age', 'notes', 'winnings', 'speed_rating', 'starts', 'win', 'thirds', 'seconds']]
